...
export const ciHealingRuns = pgTable(
  'ci_healing_runs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    prState: varchar('pr_state', { length: 30 }).notNull().default('none'),
    prBranch: varchar('pr_branch', { length: 255 }),
    aiProvider: varchar('ai_provider', { length: 50 }).notNull().default('anthropic'),
    aiModel: varchar('ai_model', { length: 100 }),
    resolvedBy: varchar('resolved_by', { length: 30 }).notNull().default('none'),
  ...
  (table) => [
    uniqueIndex('idx_ci_healing_runs_dedupe').on(table.repository, table.commitSha, table.errorHash),
    index('idx_ci_healing_runs_status').on(table.status),
    index('idx_ci_healing_runs_updated_at').on(table.updatedAt),
  ]
);

// ─── CI Healing Attempts ───────────────────────────────────────────────────

export const ciHealingAttempts = pgTable(
  'ci_healing_attempts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    runId: uuid('run_id').notNull().references(() => ciHealingRuns.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    uniqueIndex('idx_ci_healing_attempts_run_attempt').on(table.runId, table.attemptNo),
    index('idx_ci_healing_attempts_status').on(table.status),
  ]
);

export const ciHealingEvents = pgTable(
  'ci_healing_events',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    runId: uuid('run_id').notNull().references(() => ciHealingRuns.id, { onDelete: 'cascade' }),
    eventType: varchar('event_type', { length: 100 }).notNull(),
    actor: varchar('actor', { length: 30 }).notNull().default('system'),
    message: text('message').notNull(),
    payload: jsonb('payload').default(sql`'{}'::jsonb`),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    index('idx_ci_healing_events_run_id').on(table.runId),
    index('idx_ci_healing_events_event_type').on(table.eventType),
    index('idx_ci_healing_events_created_at').on(table.createdAt),
  ]
);

export const ciHealingRunsRelations = relations(ciHealingRuns, ({ many }) => ({
  attempts: many(ciHealingAttempts),
  events: many(ciHealingEvents),
}));

export const ciHealingAttemptsRelations = relations(ciHealingAttempts, ({ one }) => ({
  run: one(ciHealingRuns, { fields: [ciHealingAttempts.runId], references: [ciHealingRuns.id] }),
}));

export const ciHealingEventsRelations = relations(ciHealingEvents, ({ one }) => ({
  run: one(ciHealingRuns, { fields: [ciHealingEvents.runId], references: [ciHealingRuns.id] }),
}));
